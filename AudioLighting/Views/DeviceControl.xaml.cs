...
namespace AudioLighting.Views
{
    ...
    public partial class DeviceControl : UserControl
    {
        ...
        public int Smoothing => DeviceItem.Smoothing;

        public UdpDevice DeviceItem => (UdpDevice)MyDevice;
        ...
        public void RefAll()
        {
            if (DeviceItem.DeviceName.ToLower() == "meta")
            {
                return;
            }
        ...
        public class ddElement
        {
            ...
            public ddElement(string n, bool isStar)
            {
                ...
                if (isStar)
                {
                    p.Kind = PackIconFontAwesomeKind.StarRegular;
                }
        ...
        {
            var d = new Grid();
            var s = new PackIconFontAwesome
            {
                Kind = PackIconFontAwesomeKind.StarRegular,
            ...
            else
            {
                var l = new Label
                {
                    Content = string.Format("{0}\t{1}", p.id, p.name)
                }; d.Children.Add(l);
            }
            return d;
        }
        ...
        public string DeviceName => DeviceItem.DeviceName;

        public string Details
        {
            get
            {
                var host = DeviceItem.Hostname;
                if (!string.IsNullOrEmpty(host))
                {
                    return "Hostname: " + host + ", IP: " + DeviceItem.Ip;
                }
                else
                {
                    return "IP: " + DeviceItem.Ip;
                }
            }
        }
        ...
        private void BtnEdit_Click(object sender, RoutedEventArgs e)
        {
            ...
            var mwInstance = Window.GetWindow(this) as MainWindow;
            //DeviceItem.Stop();
            foreach (var u in MyUtils.UdpDevices)
            {
                u.Stop();
            }
        ...
        private void dropDrop_Selected(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var value = m.DataContext as string;
                var num = Convert.ToInt32(value.ToString().Substring(0, value.ToString().IndexOf('\t')));
        ...
        private void ddb_ContextMenuOpening(object sender, ContextMenuEventArgs e)
        {
            ...
            {
                if (DeviceItem.getWebserverInfo())
                {
                    FillDropDowns();
                }
            }
        ...
        private void RefreshDevice()
        {
            FillDropDowns();
            sld.Value = DeviceItem.brightness;
            if (DeviceItem.power == 1)
            {
                tbtnPower.IsChecked = true;
            }
            else
            {
                tbtnPower.IsChecked = false;
            }
        }
    }
}
