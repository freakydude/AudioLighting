...
namespace AudioLighting.Views
{
    public partial class MainWindow : MetroWindow
    {
        ...
        private bool audioDeviceLoaded;
        ...
        {
            while (!deviceListInitialized || string.IsNullOrEmpty(loadedAudioDevice))
            {
                if (string.IsNullOrWhiteSpace(loadedAudioDevice))
                {
                    return;
                }
            }
            ...
            {
                if (cboDevices.Items.Contains(loadedAudioDevice))
                {
                    cboDevices.SelectedIndex = cboDevices.Items.IndexOf(loadedAudioDevice);
        ...
        private void CboDevices_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            var s = (sender as ComboBox).SelectedItem as string;
        ...
        {
            var s = new SaveObject(MyUtils.UdpDevices);
            if (string.IsNullOrEmpty(dev))
            {
                dev = MyUtils.audioDevice;
            }
        ...
        private void sldSpeed_ValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
        {
            foreach (var u in MyUtils.UdpDevices)
            {
                u.setSpeedAsync((int)sldSpeed.Value);
            }
        }
