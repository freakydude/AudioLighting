...
namespace AudioLighting.Models
{
    public class UdpDevice : ICommunicate
    {
        ...
        public void setPowerAsync(int val)
        {
            ...
            var content = new FormUrlEncodedContent(values);
            var addr = "http://" + ip + "/power";
            httpClient.PostAsync(addr, content);
        }

        public bool getWebserverInfo()
        {
            if (deviceName.ToLower() == "meta")
            {
                return false;
            }

            var addr = "http://" + ip + "/all";
            var response = "";
            try
            {
                try
        ...
        public void EvaluateWebserverInfo(List<WebserverResponse> w)
        {
            ...
            ColorString = w[9].Value.Value.String;
            try
            {
                if (w.Count >= 17)
                {
                    lines = (int)w[16].Value.Value.Integer;
                }
            }
            catch (Exception)
            {
            ...
            foreach (var s in w[2].Options)
            {
                ...
                if (s.ToLower().Contains("visual"))
                {
                    visualizationPatterns.Add(new Pattern
                    {
                        id = idex,
                        name = s
                    });
                }
                else if (s.ToLower().Contains("twinkle"))
                {
                    twinklePatterns.Add(new Pattern
                    {
                        id = idex,
                        name = s
                    });
                ...
                {
                    regularPatterns.Add(new Pattern { id = idex, name = s });
                }
        ...
        {
            var deep = new UdpDevice(deviceName, ip, port, lines, smoothing, "")
            {
                smooth = smooth
            };
            return deep;
    ...
    public class WpfUserControlDevice : ICommunicate
    {
        ...
        private bool enable;
        private readonly Spectrum spec;
        private readonly Queue<List<byte>> lastVals = new Queue<List<byte>>();
        private int smoothing;
    ...
    {
        private readonly WpfVisualizer w;
